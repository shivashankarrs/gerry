...
def build_rnn(traindata, valdata, testdata, tokenizer, odir=tp_output_dir, LAMBDA_REVERSAL_STRENGTH = 1):
    ...
    gend_out = Dense(
        units=24, activation='softmax', name='gender_classifier'
        )(gend_in)

    model = Model(inputs=text_input, outputs=[predicts, gend_out])#eth_out
    advmodel  = Model(inputs=text_input, outputs=[gend_out])#eth_out

    layer_names = ['hatepredict', 'gender_classifier']
    loss_dict = {}
    metrics_dict = {}
    adv_loss_dict = {}
    adv_metrics_dict = {}
    ...
    for iterv in range(epochs):
        ...
        for x_train, y_train, gender_train, country_train, supertopic_train in train_iter:
            ...
            train_priv_inter = to_categorical(train_priv_inter, num_classes=24)

            tmp = model.train_on_batch(
                [x_train], [np.array(y_train), np.array(train_priv_inter)]
            )
            """advtmp = advmodel.train_on_batch(
                [x_train], [np.array(train_priv_inter)]
            )"""
        ...
        results_df = pd.DataFrame(results)
        results_df.to_csv(odir+'TP_RNN_adv_inter_MC_results_{}_{}.tsv'.format(iterv, LAMBDA_REVERSAL_STRENGTH), index=False)

    with open(odir+'validation_results_adv_TP_RNN_Inter_MC_{}.pkl'.format(LAMBDA_REVERSAL_STRENGTH), 'wb') as f:
