...
def build_mlp(xtrain, ytrain, ytrain_gender, ytrain_econ, xvalid, yvalid, yvalid_gender, yvalid_econ, xtest, ytest, ytest_gender, ytest_econ, LAMBDA_REVERSAL_STRENGTH = 1):
    ...
    gend_out = Dense(
        units=4, activation='softmax', name='domain_classifier'
        )(gend_in)

    model = Model(inputs=text_input, outputs=[predicts, gend_out])
    advmodel = Model(inputs=text_input, outputs=gend_out)

    layer_names = ['predict', 'domain_classifier']
    ...
    train_priv_inter = []
    for g, c in zip(ytrain_gender, ytrain_econ):
        train_priv_inter.append(inter_enc[(g, c)])
    train_priv_inter = to_categorical(train_priv_inter, num_classes=4)

    valid_priv_inter = []
    for g, c in zip(yvalid_gender, yvalid_econ):
        valid_priv_inter.append(inter_enc[(g, c)])
    valid_priv_inter = to_categorical(valid_priv_inter, num_classes=4)

    test_priv_inter = []
    for g, c in zip(ytest_gender, ytest_econ):
        test_priv_inter.append(inter_enc[(g, c)])
    test_priv_inter = to_categorical(test_priv_inter, num_classes=4)

    model.fit(xtrain, [ytrain, np.array(train_priv_inter)], validation_data=(xvalid, [yvalid, np.array(valid_priv_inter)]), epochs=1, verbose=0, batch_size=64)

    validation_results={}

    for iterv in range(epochs):
        history = model.fit(xtrain, [ytrain, np.array(train_priv_inter)], validation_data=(xvalid, [yvalid, np.array(valid_priv_inter)]), epochs=1, verbose=0, batch_size=64)
        #advhistory = advmodel.fit(xtrain, np.array(train_priv_inter), validation_data=(xvalid, np.array(valid_priv_inter)), epochs=1, verbose=1, batch_size=64)
        ...
        results_df.to_csv(tp_output_dir+'_adv_bios_mlp_intersectional_results_tc_{}_{}.tsv'.format(iterv, LAMBDA_REVERSAL_STRENGTH), index=False)
    
    with open(tp_output_dir+'adv_validation_intersectional_tc_{}.pkl'.format(LAMBDA_REVERSAL_STRENGTH), 'wb') as f:
