...
def constrained_optimization(xtrain, ytrain, train_gender, train_economy, xvalid, yvalid, 
    ...
    while iterv < NUM_ITERATIONS:
        ...
        results_df = pd.DataFrame(results)
        results_df.to_csv(odir+'Bios_MLP_TFCO_gerry_tw_cw_{}_results_{}_{}.tsv'.format(weighted, iterv, nu), index=False)

    with open(odir+'validation_results_Bios_MLP_TFCO_gerry_tw_cw_{}_{}.pkl'.format(weighted, nu), 'wb') as f:
