...
from gerry_eval import (
    Gerrymandering_eval,
    get_all_combs,
    Gerrymandering_groups,
    bias_attribute_selection_simple,
)
...
bios_keymapping = {
    ...
    "economy": {1: "High economy", 0: "Rest"},
}


def gerry_labels(ytrain, train_gender, train_economy):
    tasks = ["gender", "economy"]
    attribute_distinct_labels = {"gender": [0, 1], "economy": [1, 0]}
    ...
    for task_comb, group_comb in tqdm(gerry_combs):
        ...
        _lcount = [0] * len(xtrain_tp)
        lind = 0
        for gid, y in zip(group_indices, ytrain):
            if gid:
                _labels[lind] = 1
                if y == 1:
                    _lcount[lind] = 1
        ...
        label_tp.append(_lcount)
    print(len(_gerry_labels))
    return _gerry_labels, label_tp
...
def build_mlp():
    ...
    dp = Dropout(rate=0.2, name="inlp")(mlp)
...
def constrained_optimization(
    ...
    input_tensor = tf.Variable(
        np.zeros((BATCH_SIZE, MAX_SEQUENCE_LENGTH), dtype="float32"), name="input"
    )
    ...
    train_gerry, lcount = gerry_labels(ytrain, train_gender, train_economy)
    valid_gerry, _ = gerry_labels(yvalid, valid_gender, valid_economy)
    test_gerry, _ = gerry_labels(ytest, test_gender, test_economy)

    for i in range(len(train_gerry)):
        inter_data[i] = sum(train_gerry[i]) / sum(lcount[i])
    ...
    while iterv < NUM_ITERATIONS:
        train_iter = evaluator.data_iter_bios(
            xtrain, ytrain, train_gender, train_economy, batch_size=BATCH_SIZE
        )
        ...
        for x_train, y_train, gender_train, economy_train in train_iter:
            ...
            train_gerry_batch, _ = gerry_labels(y_train, gender_train, economy_train)
            input_tensor.assign(x_train)
            labels_tensor.assign(y_train)
            groups_tensor_0.assign(np.array(train_gerry_batch[0]))
            groups_tensor_1.assign(np.array(train_gerry_batch[1]))
            groups_tensor_2.assign(np.array(train_gerry_batch[2]))
            groups_tensor_3.assign(np.array(train_gerry_batch[3]))
            groups_tensor_4.assign(np.array(train_gerry_batch[4]))
            groups_tensor_5.assign(np.array(train_gerry_batch[5]))
            groups_tensor_6.assign(np.array(train_gerry_batch[6]))
            groups_tensor_7.assign(np.array(train_gerry_batch[7]))
        ...
        # model_constrained.save_weights("model_tfco"+str(iteration_count)+".h5")

        # valid model to find the best model
        print("---------------Validation------------")
        valid_iter = evaluator.data_iter_bios(
            xvalid,
            yvalid,
            valid_gender,
            valid_economy,
        ...
        test_iter = evaluator.data_iter_bios(
            xtest,
            ytest,
            test_gender,
            test_economy,
        ...
        results_df.to_csv(
            odir
            + "Bios_MLP_TFCO_gerry_tw_cw_{}_results_{}_{}.tsv".format(
                weighted, iterv, nu
            ),
            index=False,
        )
    ...
    with open(
        odir
        + "validation_results_Bios_MLP_TFCO_gerry_tw_cw_{}_{}.pkl".format(weighted, nu),
        "wb",
    ) as f:
    ...
    print(
        ...
        "best index from TFCO, not ranked",
    )
...
if __name__ == "__main__":
    ...
    for weighted in [True, False]:
        for nu in [
            ...
            model = constrained_optimization(
                xtrain,
                np.array(ytrain),
                np.array(train_gender),
                np.array(train_economy),
                xvalid,
                np.array(yvalid),
                np.array(valid_gender),
                np.array(valid_economy),
                xtest,
                np.array(ytest),
                np.array(test_gender),
                np.array(test_economy),
                nu=nu,
                weighted=weighted,
            )
