...
def constrained_optimization(
    ...
    while iterv < NUM_ITERATIONS:
        ...
        for x_train, y_train, gender_train, economy_train in train_iter:
            ...
            if x_train.shape[0] < BATCH_SIZE:
                continue
            y_tr_gender = to_categorical(gender_train, num_classes=2)
            y_tr_economy = to_categorical(economy_train, num_classes=2)
            ...
            groups_tensor_2.assign(np.array(y_tr_economy[:, 0]))
            groups_tensor_3.assign(np.array(y_tr_economy[:, 1]))
        ...
        results_df.to_csv(
            odir
            + "Bios_MLP_TFCO_independent_tw_cw_{}_results_{}_{}.tsv".format(
                weighted, iterv, nu
            ),
            index=False,
        )
    ...
    with open(
        odir
        + "validation_results_Bios_MLP_TFCO_independent_tw_cw_{}_{}.pkl".format(
            weighted, nu
        ),
        "wb",
    ) as f:
