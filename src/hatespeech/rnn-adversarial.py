...
def build_rnn(lang, odir, LAMBDA_REVERSAL_STRENGTH = 1):
    ...
    age_flip_layer = GradientReversal(LAMBDA_REVERSAL_STRENGTH)
    age_in = age_flip_layer(dp)
    age_out = Dense(
        units=1, activation='sigmoid', name='age_classifier'
        )(age_in)
    ...
    eth_out = Dense(
        units=1, activation='sigmoid', name='ethnicity_classifier'
        )(eth_in)

    model = Model(inputs=text_input, outputs=[predicts, gend_out, age_out, ctr_out, eth_out])#eth_out
    advmodel  = Model(inputs=text_input, outputs=[gend_out, age_out, ctr_out, eth_out])

    layer_names = ['hatepredict', 'gender_classifier', 'age_classifier', 'country_classifier', 'ethnicity_classifier']
    loss_dict = {}
    metrics_dict = {}
    adv_loss_dict = {}
    adv_metrics_dict = {}
    ...
    model.compile(
        ...
        metrics=metrics_dict, loss_weights = [1, 1, 1, 1, 1]
    )

    advmodel.compile(
        loss=adv_loss_dict, optimizer='rmsprop',
        metrics=adv_metrics_dict, loss_weights = [1, 1, 1, 1]
    )
    ...
    train_iter = evaluator.data_iter_adv(
    ...
    for e in range(epochs):
        ...
        for _, x_train, y_train, y_tr_gender, y_tr_age, y_tr_country, y_tr_ethnicity in train_iter:

            if len(np.unique(y_train)) == 1:
                continue
           
            tmp = model.train_on_batch(
                [x_train], [y_train, y_tr_gender, y_tr_age, y_tr_country, y_tr_ethnicity] #joint
            )
        ...
        with open(odir+'hatespeech_adv_independent_{}_{}.tsv'.format(e, LAMBDA_REVERSAL_STRENGTH), 'w') as wfile:
        ...
        evaluator.eval(
            odir+'hatespeech_adv_independent_{}_{}.tsv'.format(e, LAMBDA_REVERSAL_STRENGTH), 
            odir+'hatespeech_adv_independent_{}_{}.score'.format(e, LAMBDA_REVERSAL_STRENGTH)
        )
    
    with open(odir+'validation_hatespeech_adv_independent_{}.pkl'.format(LAMBDA_REVERSAL_STRENGTH), 'wb') as f:
