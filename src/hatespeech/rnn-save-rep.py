...
def build_rnn(lang, odir):
    ...
    for e in range(epochs):
        ...
        train_iter = evaluator.data_iter(indices_dir + "train.tsv", batch_size=64)
        ...
        for _, x_train, y_train in train_iter:
            ...
            tmp = model.train_on_batch([x_train], y_train)

        # valid model to find the best model
        print("---------------Validation------------")
        valid_iter = evaluator.data_iter(
            indices_dir + "valid.tsv", batch_size=64, if_shuffle=False
        )
        y_preds = []
        y_valids = []
    ...
    train_iter = evaluator.data_iter(
        indices_dir + "train.tsv", batch_size=64, if_shuffle=False
    )
    ...
    test_iter = evaluator.data_iter(
        indices_dir + "test.tsv", batch_size=64, if_shuffle=False
    )
